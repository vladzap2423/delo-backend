...
export class CommissionService {
  ...
  // Получить все комиссии (только с активными пользователями)
async getAllCommissions(): Promise<Commission[]> {
  return this.commissionRepo
    .createQueryBuilder("commission")
    .leftJoinAndSelect("commission.users", "user", "user.isActive = :active", { active: true })
    .getMany();
}
